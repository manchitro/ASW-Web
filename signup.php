		...
		<form action="includes/signup.inc.php" method="post">
			...
			if ($_GET) {
				if ($_GET['error'] == 'academicidexists') {
					echo "<p>Could not create account. Academic ID already belongs to an account</p>";

				}
				elseif ($_GET['error'] == 'emailexists') {
					echo "<p>Could not create account. Email already belongs to an account</p>";
				}
			}
			?>
			...
			<input class="text-email" type="email" name="email" id="email" placeholder="Academic Email (@aiub.edu)" required title="You need an aiub.edu email to sign up" pattern="^[a-zA-Z0-9]+@aiub\.edu$" value="<?php echo(isset($_GET['']))?$_GET['']:''?>">
